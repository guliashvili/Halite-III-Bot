...
template <class T> T hlt::Genes::get_arg(T s, T e, const char *args) {
...
hlt::Genes::Genes(int argc, const char *argv[]) {
  ...
  log::log("greedy_walk_randomisation_margin: " +
           to_string(greedy_walk_randomisation_margin));
  log::log("margin_to_create_new_ship: " +
           to_string(margin_to_create_new_ship));
  log::log("total_halite_margin_substr: " +
           to_string(total_halite_margin_substr));
