...
struct MapCell {
  ...
  std::shared_ptr<Entity>
      structure; // only has dropoffs and shipyards; if id is -1, then it's a
                 // shipyard, otherwise it's a dropoff
  bool collision;

  MapCell(int x, int y, Halite halite)
      : position(x, y), halite(halite), collision(false) {}
  ...
  int move_cost() const { return halite / constants::MOVE_COST_RATIO; }
  void mark_safe() {
    ship = nullptr;
    collision = false;
  }
